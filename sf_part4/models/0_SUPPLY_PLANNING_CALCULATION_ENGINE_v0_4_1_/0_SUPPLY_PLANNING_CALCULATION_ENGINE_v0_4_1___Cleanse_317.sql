...
DbFileInput_297_297 AS (
  ...
  FROM {{ prophecy_tmp_source('0_SUPPLY_PLANNING_CALCULATION_ENGINE_v0_4_1_', 'DbFileInput_297_297') }}
...
Formula_301_0 AS (
  ...
  SELECT 
    (
      CASE
        WHEN (coalesce(((POSITION(LOWER('Production - ') IN LOWER(VENDOR))) > 0), FALSE))
          THEN 'Production'
        WHEN (coalesce(((POSITION(LOWER('Transfer - ') IN LOWER(VENDOR))) > 0), FALSE))
          THEN 'Transfer  '
        ELSE NULL
      END
...
Filter_302 AS (
  ...
  WHERE (PRODUCTION_OR_TRANSFER_FLAG IS NOT NULL)
