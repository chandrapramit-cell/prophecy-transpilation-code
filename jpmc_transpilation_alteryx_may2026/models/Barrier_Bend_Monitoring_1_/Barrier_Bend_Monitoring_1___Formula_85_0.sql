...
Join_38_inner AS (
  ...
  SELECT 
    ...
    in1.Max_USDLegalRisky AS LegalslashRisky
...
Formula_34_0 AS (
  ...
  SELECT 
    ...
    CAST(ABS(CAST(LegalslashRisky AS DECIMAL (19, 9))) AS DOUBLE) AS Abs,
    *
...
AlteryxSelect_40 AS (
  ...
  SELECT 
    ...
    LegalslashRisky AS LegalslashRisky,
    Abs AS Abs,
    CheckquesMark AS CheckquesMark,
    * EXCEPT (`Imnt Id hasStaticCheckMaxBreach`, 
    `Imnt Id All`, 
    `Imnt Id`, 
    `Final Breach`, 
    `LegalslashRisky`, 
...
Join_41_left_UnionLeftOuter AS (
  ...
  SELECT 
    ...
    in0.LegalslashRisky AS LegalslashRisky,
    in0.Abs AS Abs,
    in0.CheckquesMark AS CheckquesMark,
    (
      CASE
    ...
    in0.* EXCEPT (`Imnt Id`, `Final Breach`, `LegalslashRisky`, `Abs`, `CheckquesMark`),
  ...
  SELECT 
    CAST((
      CASE
        ...
        ELSE 'TRUE'
      END
    ) AS string) AS `Accepted BreachquesMark`,
    *
...
Formula_46_1 AS (
  ...
  SELECT 
    CAST((
      CASE
        WHEN (CheckquesMark = 'FALSE')
          THEN 'No Breach'
        WHEN (`Final Breach` = 'FALSE')
          THEN 'No breach'
        WHEN (`Accepted BreachquesMark` = 'TRUE')
...
Join_77_left_UnionLeftOuter AS (
  ...
  SELECT 
    in0.`Accepted BreachquesMark` AS `Accepted BreachquesMark`,
    in0.LegalslashRisky AS LegalslashRisky,
    in0.`Imnt Id` AS `Imnt Id`,
