...
Filter_71_reject_to_Filter_72 AS (
  ...
  WHERE (
          (
            (
              (
                NOT(
                  `Org Level 11 Description` = 'CURRENCIES AND EMERGING MARKETS')
              )
              OR ((`Org Level 11 Description` = 'CURRENCIES AND EMERGING MARKETS') IS NULL)
            )
            AND (
                  (
                    NOT(
                      `Org Level 11 Description` = 'GLOBAL RATES & RATES EXOTICS')
                  )
                  OR ((`Org Level 11 Description` = 'GLOBAL RATES & RATES EXOTICS') IS NULL)
                )
          )
          AND (`Org Level 11 Description` = 'GLOBAL COMMODITIES')
        )
...
AlteryxSelect_74 AS (
  ...
  FROM Filter_71_reject_to_Filter_72 AS in0

)

SELECT *

FROM AlteryxSelect_74
