...
WITH Directory_51 AS (

  SELECT *
  
  FROM {{ prophecy_tmp_source('CEM_Securities_PruVal_6m_Trade_Volume_Upload_v_1_0_1_', 'Directory_51') }}

),

Directory_51_reformat AS (
...
AlteryxSelect_52 AS (
  ...
  FROM Directory_51_reformat AS in0
...
Macro_76 AS (
  ...
  {{
    prophecy_basics.ToDo(
      'Failed to parse Macro. Please upload the file 2.Systems/CSV_load.yxmc to resolve it.'
    )
  }}
...
Configuration_t_58 AS (

  SELECT *
  
  FROM {{ prophecy_tmp_source('CEM_Securities_PruVal_6m_Trade_Volume_Upload_v_1_0_1_', 'Configuration_t_58') }}
