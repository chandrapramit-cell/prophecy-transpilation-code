...
Formula_44_1 AS (
  ...
  SELECT 
    CAST(((CAST(Sum_US AS INT64) / TotalUpperShots) * 100) AS FLOAT64) AS UpperShotAccuracy,
    CAST(((CAST(Sum_LS AS INT64) / TotalLowerShots) * 100) AS FLOAT64) AS LowerShotAccuracy,
...
Formula_44_2 AS (
  ...
  SELECT 
    CAST((((CONCAT(Sum_US, Sum_LS)) / TotalShots) * 100) AS FLOAT64) AS TotalShotAccuracy,
    *
  
  FROM Formula_44_1 AS in0
