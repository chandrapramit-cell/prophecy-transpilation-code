...
Filter_63 AS (
  ...
  WHERE (in0.ToslashCc = 'Cc')

),

Summarize_66 AS (

  SELECT (CONCAT_WS(';', (COLLECT_LIST(in0.`Email address`)))) AS `Email address`
...
Unique_51 AS (
  ...
  QUALIFY ROW_NUMBER() OVER (PARTITION BY in0.FISCAL_YEAR, in0.PERIOD, in0.LEDGER, in0.COMPANY_CODE ORDER BY in0.FISCAL_YEAR, in0.PERIOD, in0.LEDGER, in0.COMPANY_CODE) = 1
