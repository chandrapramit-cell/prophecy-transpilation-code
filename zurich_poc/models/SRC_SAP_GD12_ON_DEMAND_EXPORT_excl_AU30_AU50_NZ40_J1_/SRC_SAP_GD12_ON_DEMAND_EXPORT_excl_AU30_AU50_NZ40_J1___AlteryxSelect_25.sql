...
RegEx_42_rename_0 AS (
  ...
  SELECT 
    in0.FileName_replaced AS FileName,
...
DynamicRename_26_after AS (
  ...
  SELECT 
    in0.SEND__BUSAREA AS Send__BusArea,
    in0.BUS__TRANSACTION AS Bus__transaction,
...
AlteryxSelect_25 AS (
  ...
  SELECT 
    in0.variableTIMESTAMP AS variableTIMESTAMP,
    USER AS USER,
    in0.SOURCE AS SOURCE,
    in0.FILENAME AS FILENAME,
    in0.PERIOD AS PERIOD,
    in0.LEDGER AS LEDGER,
    in0.FILELASTWRITETIME AS FILELASTWRITETIME,
    in0.CC_CHECK AS CC_CHECK,
