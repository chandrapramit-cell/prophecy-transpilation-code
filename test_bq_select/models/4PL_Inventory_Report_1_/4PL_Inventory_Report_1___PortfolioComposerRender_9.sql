...
ItemNumberUploa_75 AS (
  ...
  FROM {{ prophecy_tmp_source('4PL_Inventory_Report_1_', 'ItemNumberUploa_75') }}
...
Filter_76 AS (
  ...
  WHERE (
          ...
          AND ((CAST(TRIM(ItemNumber) AS STRING) <> '') OR (TRIM(ItemNumber) IS NULL))
        )
