...
with Pipeline(args) as pipeline:
    ...
    _11_0_1__database___generate_monthly_financialsm_rep_non_sunday_check_sq_dbattribute = Process(
        name = "{11_0_1}_Database___Generate_Monthly_Financialsm_rep_non_sunday_check_SQ_DBATTRIBUTE",
        properties = OracleSource(
          ...
          properties = OracleSource.OracleSourceInternal(
            ...
            warehouseQuery = OracleSource.WarehouseQuery(
              query = "SELECT /*+ parallel(db) */ rtrim(to_char(to_date(currvalue, 'YYYYMMDD'), 'DAY')) AS day_of_week FROM dbattribute db WHERE name = 'businessdate' AND DAY_OF_WEEK != 'SATURDAY'"
            )
          ),
          format = OracleSource.OracleReadFormat(
            schema = "external_sources/m_rep_non_sunday_check/{11_0_1}_Database___Generate_Monthly_Financialsm_rep_non_sunday_check_SQ_DBATTRIBUTE.yml"
          )
        ),
        input_ports = None,
    ...
    (
        _11_0_1__database___generate_monthly_financialsm_rep_non_sunday_check_sq_dbattribute
        >> m_rep_non_sunday_check__rep_inf_workflow
    )
