{{
  config({    
    "materialized": "table",
    "alias": "intermediate_productos_AlteryxSelect_1210",
    "database": "tanmay",
    "schema": "default"
  })
}}
...
WITH aka_Server_UYDB_1043 AS (
  ...
  FROM {{ prophecy_tmp_source('productos_part1', 'aka_Server_UYDB_1043') }}
...
Formula_1230_0 AS (
  ...
  SELECT 
    CAST((
      CASE
        WHEN CAST((coalesce((CONTAINS(LOWER(datos_adicionales), LOWER('BPS'))), FALSE)) AS BOOLEAN)
          THEN 'BPS'
        WHEN CAST((coalesce((CONTAINS(LOWER(datos_adicionales), LOWER('DGI'))), FALSE)) AS BOOLEAN)
          THEN 'DGI'
        ELSE 'Otros'
      END
