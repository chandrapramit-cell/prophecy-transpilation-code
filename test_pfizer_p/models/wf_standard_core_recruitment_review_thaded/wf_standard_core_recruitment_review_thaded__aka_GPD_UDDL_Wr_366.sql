  ...
  SELECT 
    CAST((
      CASE
        ...
        ELSE 'ZZZ'
      END
    ) AS VARCHAR (1000)) AS demography_diversity,
    CAST(CASE
...
Join_531_left_UnionFullOuter AS (
  ...
  {#Combines two datasets with a full join while excluding sensitive demographic fields to support policy analysis.#}
  SELECT 
    in0.*,
